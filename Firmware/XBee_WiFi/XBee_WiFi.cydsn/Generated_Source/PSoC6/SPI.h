...
#define SPI_SPI_SLAVE_SELECT1    (SPI_SCB__SS1_POSITION)

/** The slave select line 2 constant which takes into account pin placement */
#define SPI_SPI_SLAVE_SELECT2    (SPI_SCB__SS2_POSITION)

/** The slave select line 3 constant which takes into account pin placement */
#define SPI_SPI_SLAVE_SELECT3    (SPI_SCB__SS3_POSITION)
/** @} group_macros */
...
__STATIC_INLINE uint32_t SPI_ReadArray(void *rxBuf, uint32_t size)
{
    return Cy_SCB_SPI_ReadArray(SPI_HW, rxBuf, size);
}
...
__STATIC_INLINE uint32_t SPI_WriteArray(void *txBuf, uint32_t size)
{
    return Cy_SCB_SPI_WriteArray(SPI_HW, txBuf, size);
}
...
__STATIC_INLINE void SPI_WriteArrayBlocking(void *txBuf, uint32_t size)
{
    Cy_SCB_SPI_WriteArrayBlocking(SPI_HW, txBuf, size);
}
...
__STATIC_INLINE cy_en_scb_spi_status_t SPI_Transfer(void *txBuf, void *rxBuf, uint32_t size)
{
    return Cy_SCB_SPI_Transfer(SPI_HW, txBuf, rxBuf, size, &SPI_context);
}
