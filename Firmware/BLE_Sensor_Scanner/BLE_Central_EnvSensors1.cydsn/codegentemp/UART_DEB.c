...
*/
cy_stc_scb_uart_config_t const UART_DEB_config =
{
...
* structure.
*/
cy_stc_scb_uart_context_t UART_DEB_context;
...
*
* \globalvars
* \ref UART_DEB_initVar - used to check initial configuration,
...
void UART_DEB_Start(void)
{
    ...
    #if defined(UART_DEB_SCB_IRQ__INTC_ASSIGNED)
        (void) Cy_SysInt_Init(&UART_DEB_SCB_IRQ_cfg, &UART_DEB_Interrupt);
    #endif /* (UART_DEB_SCB_IRQ__INTC_ASSIGNED) */
...
#if defined(UART_DEB_SCB_IRQ__INTC_ASSIGNED)
    NVIC_EnableIRQ((IRQn_Type) UART_DEB_SCB_IRQ_cfg.intrSrc);
#endif /* (UART_DEB_SCB_IRQ__INTC_ASSIGNED) */
