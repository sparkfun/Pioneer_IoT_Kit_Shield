...
#else
    #define I2C_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!I2C_CY_SCBIP_V0) */

static uint32 I2C_I2CMasterHandleStatus(uint32 status);
static uint32 I2C_I2CMasterWaitOneUnit(uint32 *timeout);
...
uint32 I2C_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    if(I2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        I2C_SetMasterInterruptMode    (I2C_I2C_INTR_MASTER_MASK);
...
{
    uint32 errStatus = I2C_I2C_MSTR_NOT_READY;
    ...
    if(I2C_CHECK_I2C_FSM_IDLE)
    {
        ...
        {
            uint32 locStatus;
            uint32 timeout = I2C_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            I2C_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            I2C_SetMasterInterruptMode    (I2C_NO_INTR_SOURCES);
            I2C_ClearMasterInterruptSource(I2C_INTR_MASTER_ALL);
            ...
            do
            {
                locStatus  = I2C_GetMasterInterruptSource() & I2C_INTR_MASTER_SEND_BYTE;
                locStatus |= I2C_GetSlaveInterruptSource() & I2C_SLAVE_INTR_I2C_ADDR;
                locStatus |= I2C_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = I2C_I2CMasterHandleStatus(locStatus);
        }
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See I2C_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
uint32 I2C_I2CMasterSendStop(uint32 timeoutMs)
{
    ...
    if(I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2C_GetMasterInterruptSource() & I2C_INTR_MASTER_SEND_STOP;
            locStatus |= I2C_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
uint32 I2C_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    ...
    if(I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2C_GetMasterInterruptSource() & I2C_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = I2C_GetRxInterruptSource() & I2C_INTR_RX_NOT_EMPTY;
            locStatus |= I2C_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)I2C_RX_FIFO_RD_REG;
            I2C_ClearRxInterruptSource(I2C_INTR_RX_ALL);
        }
...
void I2C_I2CReStartGeneration(void)
{
    ...
    */
    ...
    uint32 timeout = I2C_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = I2C_I2C_STATUS_REG & I2C_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((I2C_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = I2C_I2C_PHASE_GEN_TIMEOUT;
        ...
        if (0u != readDirection)
        {
            ...
            I2C_SET_I2C_SCL_DR(I2C_I2C_SCL_LOW);
            CyDelayUs(I2C_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            I2C_SET_I2C_SCL_DR(I2C_I2C_SCL_HIGH);
            CyDelayUs(I2C_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((I2C_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
...
/*******************************************************************************
* Function Name: I2C_I2CMasterHandleStatus
****************************************************************************//**
*
...
* I2C_I2C_MASTER_TIMEOUT and I2C_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See I2C_I2CMasterSendStart() function for constants.
...
static uint32 I2C_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & I2C_I2C_MASTER_TIMEOUT))
    {
        retStatus  = I2C_I2C_MASTER_TIMEOUT;
        resetBlock = I2C_I2C_RESET_ERROR;
    }
    else if (0u != (I2C_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = I2C_I2C_MSTR_ERR_ABORT_START;

        I2C_I2C_MASTER_CMD_REG = 0u;

        I2C_state = I2C_I2C_FSM_IDLE;
    }
    else if (0u != (status & I2C_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = I2C_I2C_MSTR_ERR_ARB_LOST;

        if (I2C_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (I2C_I2C_CTRL_REG & I2C_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = I2C_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & I2C_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = I2C_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = I2C_I2C_RESET_ERROR;
    }
    else if (0u != (status & I2C_INTR_MASTER_I2C_NACK))
    {
        retStatus = I2C_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = I2C_I2C_MSTR_NO_ERROR;

        if (0u != (status & I2C_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (I2C_CHECK_I2C_FSM_ADDR)
            {
                ...
                I2C_state = (I2C_CHECK_I2C_FSM_RD) ?
                                          I2C_I2C_FSM_MSTR_RD_DATA : I2C_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (I2C_I2C_RESET_ERROR == resetBlock)
    {
        I2C_I2CFwBlockReset();
    }
    else
